...
import {
  ...
  type NonTransactionSubType,
} from "../api/mockActivity"
import { searchUser } from "../api/users"
import { UserActivityFilters } from "../components/Filter/UserActivityFilter"
...
export default function Users() {
  ...
  const [nonTransactionSubType, setNonTransactionSubType] = useState<NonTransactionSubType | "">("")
  ...
  const filteredActivities = mockActivities.filter((activity) => {
    if (searchResults.length === 1 && activity.userId !== searchResults[0].id) {
      return false
    }
    ...
    if (activityType === "Transactions" && transactionSubType && activity.subType !== transactionSubType) {
      return false
    }

    if (activityType === "Non-Transactions" && nonTransactionSubType && activity.subType !== nonTransactionSubType) {
      return false
    }
      ...
      <div className={container}>
        ...
        <div className={searchContainerStyles}>
         ...
         {displayedSearchQuery && searchResults.length === 1 ? (
          ...
          <UserTable searchQuery={displayedSearchQuery} onResultsChange={setSearchResults} />
        )} 
        ...
        <h2 >User Activity</h2>
         <UserActivityFilters
          ...
          onTransactionSubTypeChange={setTransactionSubType}
          onNonTransactionSubTypeChange={setNonTransactionSubType}
          onDateRangeChange={handleDateRangeChange}
        />
        <UserTransactionsTable activities={filteredActivities} />
      </div>
    </Layout>
  )
}
